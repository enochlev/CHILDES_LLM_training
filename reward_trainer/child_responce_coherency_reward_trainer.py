...
score_column = "child_coherence_score"
...
train_samples = [
    InputExample(texts=[row['text'], row["CHI_response"]], label=float(row[score_column]))
...
binary_eval_samples = [
    InputExample(texts=[row['text'], row["CHI_response"]], label=1 if row[score_column] > 0.5 else 0)
    for _, row in test_df.iterrows()
]
...
model.fit(
    ...
    output_path="../models/child_coherence_model",
...
model = CrossEncoder('../models/child_coherence_model',max_length=96)
